...
class GaOptimizer_GPU_SandF:
    ...
    def __init__(self, z_params, freq, f_idx_list,
        ...
        Args:
            ...
            freq_band:            (f_min, f_max) target frequency band [Hz] for S11.
            s11_threshold:        S11 threshold in dB for the penalty function.
        """
        ...
        f_len = self.n_freq - 1
        self.L1 = int(round((freq_band[0] - f_min.item()) /
                            (f_max.item() - f_min.item()) * f_len))
        self.L2 = int(round((freq_band[1] - f_min.item()) /
                            (f_max.item() - f_min.item()) * f_len))
    ...
    def calculate_cost(self, population_list):
        ...
        diff = target_S - self.s11_threshold
        penalty_per_point = torch.relu(diff)          # ReLU penalty
        cost_s = 1000*torch.sum(penalty_per_point, dim=1)
