...
class GAProgressPlotter:
    ...
    def __init__(self, n_generations, save_path=None, title='GA Optimization Progress'):
        ...
        plt.ion()
        self.fig, self.ax = plt.subplots(1, 1, figsize=(10, 5.5))
        self.ax.set_title(self.title, fontsize=13, fontweight='bold')

        self.ax.set_xlabel('Generation')
        self.ax.set_ylabel('Cost (Loss)')
        self.ax.grid(True, alpha=0.3)
        self.ax.set_xlim(0, n_generations)

        self.line_min, = self.ax.plot([], [], 'b-', linewidth=2,
                                       label='Min Loss')
        self.line_avg, = self.ax.plot([], [], 'r--', linewidth=1.5,
                                       label='Avg Loss')
    ...
    def update(self, gen, min_cost, avg_cost):
        ...
        self.line_min.set_data(self.generations, self.min_costs)
        self.line_avg.set_data(self.generations, self.avg_costs)
        self.ax.relim()
...
def plot_optimization_progress(logbook):
    ...
    ax.set_title('GA Optimization — Convergence', fontsize=13, fontweight='bold')

    ax.plot(gen, min_cost, 'b-', linewidth=2, label='Min Loss')
    ax.plot(gen, avg_cost, 'r--', linewidth=2, label='Avg Loss')
    ...
    ax.annotate(f'Best: {min_cost[best_idx]:.2f} (Gen {gen[best_idx]})',
